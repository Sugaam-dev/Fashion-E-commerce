export const COLLECTION_CATEGORIES = [
  {
    name: "Sarees",
    image: "https://5.imimg.com/data5/SELLER/Default/2023/10/355023483/QE/GS/LT/94678823/cotton-silk-sharee-500x500.jpeg",
    subcats: ["Paithani", "Maheshwari", "Fancy", "Kota Doriya", "Ikat", "Ilkal"],
  },
  {
    name: "Dress Material",
    image: "https://peachmode.com/cdn/shop/products/navy-blue-printed-poly-cotton-dress-material-peachmode-1.jpg?v=1669043805&width=2000",
    subcats: ["Ikat", "Jamdani", "Kota Doriya", "Maheshwari", "Narayan Peth"],
  },
  {
    name: "Readymade",
    image: "https://images.unsplash.com/photo-1572804013427-4d7ca7268217?auto=format&fit=crop&w=600&q=80",
    subcats: ["Halter Neck Short Top", "Kurti with Dupatta", "One Piece"],
  },
  {
    name: "Accessories",
    image: "https://images.unsplash.com/photo-1584917865442-de89df76afd3?auto=format&fit=crop&w=600&q=80",
    subcats: ["Paithani Clutches", "Pouches", "Sling Bags"],
  },
];

export const CATEGORIES = [
  { name: "Sarees", count: "38 Styles", type: "saree", bg: "#7a5c8c", accent: "#e9c9a5", image: "https://5.imimg.com/data5/SELLER/Default/2023/10/355023483/QE/GS/LT/94678823/cotton-silk-sharee-500x500.jpeg" },
  { name: "Dress Material", count: "61 Styles", type: "dressmaterial", bg: "#bd5b34", accent: "#e9c9a5", image: "https://images.unsplash.com/photo-1558171813-0e3a28c4b59e?auto=format&fit=crop&w=600&q=80" },
  { name: "Readymade", count: "15 Styles", type: "readymade", bg: "#4a7a1e", accent: "#f6efe4", image: "https://images.unsplash.com/photo-1572804013427-4d7ca7268217?auto=format&fit=crop&w=600&q=80" },
  { name: "Accessories", count: "15 Styles", type: "accessory", bg: "#8a9a7e", accent: "#f6efe4", image: "https://images.unsplash.com/photo-1584917865442-de89df76afd3?auto=format&fit=crop&w=600&q=80" },
];
